...
- Desktop and shell configs sit under `hypr/`, `waybar/`, `kitty/`, `tmux.conf`, `zshrc`, and `aliases`; editor config is in `nvim/` with plugin pins in `nvim/lazy-lock.json`.
- Apps and helpers include `wayland/*` overrides and `fonts/` for local fonts. Place new app configs under `~/.config`-style mirrors to match the existing linking pattern.
...
- Lua (Neovim): four-space indent, single quotes, modules return tables; keep plugin declarations consistent with existing structure and update `nvim/lazy-lock.json` when versions change.
